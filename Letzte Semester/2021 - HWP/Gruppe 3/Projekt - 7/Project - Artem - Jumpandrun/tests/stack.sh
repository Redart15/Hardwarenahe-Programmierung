#!/usr/bin/env bash

program='./jumpandrun_fsanitize'

rm -rf ausgabe
rm -rf diff
mkdir -p diff
mkdir -p ausgabe

printf '\n'
printf 'Die Stackgröße wird jetzt beschränkt. Code, der zu große Arrays auf dem Stack anlegt,\n'
printf 'anstatt diese dynamisch mit malloc zu verwalten, sollte deshalb fehlschlagen.\n'
printf 'Aufgrund der Größe der Testlevel kann dieser Schritt bei ineffizientem Code sehr lange dauern.\n'
printf 'Zum Bestehen darf "make run" nicht länger als 5 Minuten benötigen.\n\n'

function check_stack() {
    level="$1"
    input="$2"
    output="$3"
    expected="$4"
    diff_file="diff/stack.txt"
    
    # Limit stack size
    if ! ulimit -Ss 64
    then
        printf '\tFEHLER: Das Programm "ulimit" funktioniert nicht richtig. Dieser Fehler sollte eigentlich nicht auftreten.\n'
        exit 6
    fi

    (set -x ; "$program" "$input" "$output" "$level")

    return_value="$?"

    # Reset stack size limit to default (probably 8192k)
    ulimit -Ss unlimited

    if [ $return_value -eq 0 ]; then
        printf '\tOK: Rückgabewert ist 0.\n'
    else
        printf '\tFEHLER: Rückgabewert ist nicht 0, sondern %s.\n' "$return_value"
        exit 6
    fi

    if diff -yZ "$output" "$expected" > "$diff_file"
    then
        printf '\tOK: %s stimmt mit %s überein.\n' "$expected" "$output"
    else
        printf '\tFEHLER: %s stimmt nicht mit %s überein. Details in %s.\n' "$expected" "$output" "$diff_file"
        exit 7
    fi

    printf '\n'
}

check_stack 'level/10txt' 'eingabe/10txt' 'ausgabe/10txt' 'erwartete_ausgabe/10txt'
check_stack 'level/1.txt' 'eingabe/1.txt' 'ausgabe/1.txt' 'erwartete_ausgabe/1.txt'
check_stack 'level/9.dateiendungen_testen_gilt_nicht' 'eingabe/9' 'ausgabe/9.foo' 'erwartete_ausgabe/9.txt'
check_stack 'level/lang.txt' 'eingabe/lang.txt' 'ausgabe/lang.txt' 'erwartete_ausgabe/lang.txt'

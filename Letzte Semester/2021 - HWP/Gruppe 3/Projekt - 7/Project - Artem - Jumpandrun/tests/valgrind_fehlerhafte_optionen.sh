...
function test_params() {
    ...
    (set -x; valgrind --leak-check=full --show-leak-kinds=all --log-file=$logfile ./jumpandrun "${@:2}" 2> "$stderrfile")
    return_value=$?
...
test_params '1' "$doesnotexist" "$output" "$level"

printf 'Prüfe Behandlung von nicht existierendem Level.\n\n'
test_params '1' "$input" "$output" "$doesnotexist"
...
test_params '1' "$input" "$invalid" "$level"

printf 'Prüfe Behandlung von nicht-lesbarer Eingabedatei "eingabe" (ist ein Ordner).\n\n'
test_params '2' 'eingabe' "$output" "$level"

printf 'Prüfe Behandlung von nicht-lesbarer Level-Datei "level" (ist ein Ordner).\n\n'
test_params '2' "$input" "$output" 'level'
